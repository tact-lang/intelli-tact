    ...
    override fun execute(executor: Executor, runner: ProgramRunner<*>): ExecutionResult? {
        val state = object : CommandLineState(env) {
            override fun startProcess(): ProcessHandler {
                ...
                return builcContract(project, conf.fileName)
            }
        }
...
fun builcContract(
    ...
    val (contractPath, projectRoot) = tryRelativizePath(project, filepath)
    ...
    handler.addProcessListener(object : ProcessListener {
        ...
        private fun moveBuildArtifacts() {
            val root = Path(filepath).parent.toString()
            val filename = Path(filepath).fileName.toString()

            val extensions = "(abi|code\\.boc|fc|fif|md|pkg|ts)"
            val contracts = runReadAction { definedContracts(project, filepath) }
            ...
            VfsUtil.markDirtyAndRefresh(false, true, true, srcDir.toFile())
            onReady()
        }
    })
...
private fun definedContracts(project: Project, filepath: String): List<String> {
    val file = VfsUtil.findFile(Path(filepath), true) ?: return emptyList()
    val psiFile = file.findPsiFile(project) as? TactFile ?: return emptyList()
